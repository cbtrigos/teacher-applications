...
connection.connect(function(err) {
  ...
  exports.approverRegistration = function(req,res){
    ...
    // const emis_code = req.body.emis_code


    connection.query('SELECT * FROM users WHERE email = ?',[registration['email']], function (error, results, fields) {
    if(results.length >0) {
      res.status(201).send("Email already in use. Try logging in.")
        }
    else {
        connection.query('INSERT INTO users SET ?',registration, function (error, results, fields) {
        if (error) {
          res.status(400).send("An error occured registering you. Please refresh and try again. ")
            ...
            connection.query('SELECT * FROM users WHERE email = ?',[registration['email']], function (error, results, fields) {
              if (error) {
                ...
                if(results.length >0){ // if you were registered properly, then insert into the approval requests
                  connection.query('INSERT INTO approver_requests SET ?, user_id = ?',[approvalRequest, results[0].user_id], function (error, results, fields) {
                  if (error) {
                    res.status(400).send("An error occured submitting your request. Please refresh and try again. ")
                  }
                  else {
                    res.status(200).send(
                      {message:'user registered sucessfully', 
