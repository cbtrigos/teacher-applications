...
exports.getallApplications = function (req, res) {
  ...
  connection.query('SELECT * FROM users WHERE user_id = ?', user_id, (
    ...
    if (error) {
      ...
      connection.query(
        'SELECT * FROM applications apps LEFT JOIN approver_1 a1 ON apps.application_id=a1.app_id LEFT JOIN approver_2 a2 ON apps.application_id=a2.app_id where apps.submitted=?', 'true',
        (error, results) => {
          if (error) {
            res.status(400).send('failed to pull applications');
          } else {
...
connection.query('SELECT * FROM users WHERE user_id = ?', user_id, (
  ...
  if (error) {
    ...
    connection.query(
      'SELECT * FROM users where user_id=?', searched,
      (error, results) => {
        if (error) {
...
exports.getOpening = function (req, res) {
  ...
  connection.query('SELECT * FROM users WHERE user_id = ?', user_id, (
    ...
    if (error) {
      ...
      connection.query(
        'SELECT * FROM job_openings where opening_key=?', searched,
        (error, results) => {
          if (error) {
                ...
                (error, results) => {
                  if (error) {
                    ...
                    transporter.sendMail(mailOptions, (err) => {
                      if (err) {
                        return res.status(200).send('Email failed to send to contact but it was approved..',
                        );
                      }
              ...
              connection.query(
                ...
                (error, results) => {
                  if (error) {
                    ...
                    transporter.sendMail(mailOptions, (err) => {
                      if (err) {
                        return res.status(200).send('Email failed to send to contact but it was rejected..',
                        );
                      }
                      return res.status(200).send('Success');
